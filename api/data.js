...
async function all(){
  ...
  const q=await Promise.all([
    ...
    db.query('SELECT id,offer_id AS "offerId",group_id AS "groupId",status,scheduled_at AS "scheduledAt",published_at AS "publishedAt",clicks,message,image_url AS "imageUrl",mode,attempts,error_message AS "errorMessage",created_at AS "createdAt" FROM publications ORDER BY created_at DESC LIMIT 250'),
    ...
    db.query('SELECT id,name,category_id AS "categoryId",start_time AS "startTime",end_time AS "endTime",interval_minutes AS "intervalMinutes",status,created_at AS "createdAt" FROM queues ORDER BY created_at DESC'),
...
export default async function(req,res){
  ...
  try{
    ...
    if(req.method==='POST'&&entity==='category'){
      ...
      await db.query('INSERT INTO publications(id,offer_id,group_id,status,scheduled_at,message,image_url,mode) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',[uid(),offerId,groupId,b.scheduledAt?'SCHEDULED':'READY',b.scheduledAt||null,txt(b.message,3000)||found.rows[0].message,found.rows[0].image_url,txt(b.mode,30)||'ON_DEMAND']);
      ...
      await db.query('INSERT INTO queues(id,name,category_id,start_time,end_time,interval_minutes,status) VALUES($1,$2,$3,$4,$5,$6,$7)',[uid(),txt(b.name,120),txt(b.categoryId,80)||null,txt(b.startTime,5)||'08:00',txt(b.endTime,5)||'22:00',Math.max(1,Math.min(30,Number(b.intervalMinutes)||10)),'ACTIVE']);
