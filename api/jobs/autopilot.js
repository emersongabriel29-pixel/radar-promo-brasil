...
export default async function(req,res){
  ...
  for(const account of accounts){
    ...
    const offers=(await db.query("SELECT * FROM offers WHERE account_id=$1 AND status='APPROVED' ORDER BY created_at DESC LIMIT 100",[accountId])).rows;
    ...
    for(const offer of offers){
      ...
      if(!rule)continue;
      let today=Number((await db.query("SELECT COUNT(*)::int AS count FROM publications WHERE account_id=$1 AND created_at>=date_trunc('day',now())",[accountId])).rows[0]?.count||0);
      const dailyLimit=Math.max(1,Number(rule.max_publications_per_day||20));
      if(today>=dailyLimit)break;
      const groups=(await db.query("SELECT id,platform FROM (SELECT id,platform,category_id,row_number() OVER(PARTITION BY platform ORDER BY CASE WHEN category_id=$2 THEN 0 ELSE 1 END,id) AS position FROM promo_groups WHERE account_id=$1 AND status='ACTIVE' AND platform IN ('WHATSAPP','TELEGRAM') AND NULLIF(trim(external_id),'') IS NOT NULL AND (category_id=$2 OR category_id IS NULL)) destinations WHERE position=1",[accountId,offer.category_id])).rows;
