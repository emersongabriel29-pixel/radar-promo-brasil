...
export default async function(req, res) {
  const due = await db.query("SELECT p.id,p.account_id FROM publications p JOIN promo_groups g ON g.id=p.group_id AND g.account_id=p.account_id LEFT JOIN account_settings s ON s.account_id=p.account_id WHERE p.status='SCHEDULED' AND p.scheduled_at<=now() AND g.status='ACTIVE' AND NULLIF(trim(g.external_id),'') IS NOT NULL AND (NOT EXISTS(SELECT 1 FROM queues q WHERE q.account_id=p.account_id AND q.status='ACTIVE') OR EXISTS(SELECT 1 FROM queues q WHERE q.account_id=p.account_id AND q.status='ACTIVE' AND (q.category_id IS NULL OR q.category_id=g.category_id) AND (now() AT TIME ZONE COALESCE(s.timezone,'America/Sao_Paulo'))::time BETWEEN q.start_time::time AND q.end_time::time AND NOT EXISTS(SELECT 1 FROM publications recent WHERE recent.account_id=p.account_id AND recent.group_id=p.group_id AND recent.published_at>now()-(q.interval_minutes||' minutes')::interval))) ORDER BY p.priority DESC,p.scheduled_at LIMIT 1");
