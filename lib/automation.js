import { formatPromo } from './promo-message.js';
...
export function categoryHint(title,source=''){
  const t=(clean(title,220)+' '+clean(source,80)).toLowerCase();
  const rules=[
    ['Produtos importados',/importad|aliexpress|shein|temu/],
...
export function message(v){return formatPromo({...v,category:categoryHint(v.title,v.source)}).message}
