...
-- Existing SaaS identifiers are TEXT, so keep this migration compatible and idempotent.
...
CREATE TABLE IF NOT EXISTS autopilot_rules (
  ...
  min_score INTEGER NOT NULL DEFAULT 85 CHECK(min_score BETWEEN 0 AND 100),
  min_discount INTEGER NOT NULL DEFAULT 15 CHECK(min_discount BETWEEN 0 AND 100),
  max_price BIGINT,
  max_publications_per_day INTEGER NOT NULL DEFAULT 5 CHECK(max_publications_per_day BETWEEN 1 AND 500),
